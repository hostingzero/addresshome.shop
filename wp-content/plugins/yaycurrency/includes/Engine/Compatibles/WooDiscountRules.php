...
class WooDiscountRules {
	...
	public function caculate_price_with_set_discount_type( $price_fixed, $quantity, $discount_lines, $set_discount ) {
	...
	// Caculate Cart Discount Price wit Set Discount Type
		...
		if ( ! empty( $cart_item['data'] ) ) {
			...
			if ( $discount ) {
				...
				if ( isset( $discount['total_discount_type_type'] ) && $set_discount_discounted_price ) {
					$discount_price_fixed                              = $this->caculate_price_with_set_discount_type( $product_fixed_price, $cart_item['quantity'], $calculated_cart_item_discount['discount_lines'], $discount );
