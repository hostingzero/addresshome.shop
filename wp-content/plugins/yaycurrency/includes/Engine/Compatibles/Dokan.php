...
class Dokan {
	...
	public function __construct() {
		...
		// REVERT TO DEFAULT SYMBOLS ON EDIT PRODUCT PAGE AND NEW PRODUCT POPUP IN DOKAN
		...
		// CUSTOM ORDER DETAILS IN DOKAN DASHBOARD
		if ( isset( $_REQUEST['order_id'] ) && isset( $_REQUEST['_wpnonce'] ) ) {
			add_action( 'wp_head', array( $this, 'wp_head' ) );

			add_filter( 'woocommerce_order_subtotal_to_display', array( $this, 'custom_formatted_order_subtotal' ), 20, 3 );
			add_filter( 'woocommerce_order_discount_to_display', array( $this, 'custom_woocommerce_order_discount_to_display' ), 20, 2 );
			add_filter( 'woocommerce_order_shipping_to_display', array( $this, 'custom_woocommerce_order_shipping_to_display' ), 20, 3 );

			add_filter( 'woocommerce_get_formatted_order_total', array( $this, 'custom_woocommerce_get_formatted_order_total' ), 10, 4 );
			// CUSTOM ORDER REFUND, FEES
			add_filter( 'woocommerce_get_order_item_totals', array( $this, 'custom_format_order_item_totals' ), 20, 3 );
		}
		// PAYMENT GATEWAY

		add_filter( 'wc_stripe_hide_display_order_fee', array( $this, 'custom_hide_display_stripe_order' ), 10, 2 );
		add_filter( 'wc_stripe_hide_display_order_payout', array( $this, 'custom_hide_display_stripe_order' ), 10, 2 );
		add_filter( 'woocommerce_paypal_payments_show_fees_on_order_admin_page', array( $this, 'custom_paypal_payments_show_fees' ), 10, 2 );
		add_filter( 'woocommerce_admin_order_totals_after_total', array( $this, 'custom_admin_order_totals_after_total' ), 10, 1 );
		...
		add_action( 'wp_ajax_yay_dokan_custom_order_details', array( $this, 'custom_yay_dokan_custom_order_details' ) );
		add_action( 'wp_ajax_nopriv_yay_dokan_custom_order_details', array( $this, 'custom_yay_dokan_custom_order_details' ) );
	...
	public function convert_price_to_default_currency( $price ) {
		$apply_default_currency = YayCurrencyHelper::filtered_by_currency_code( $this->default_currency, $this->converted_currency );
		$price                  = number_format( $price, $apply_default_currency['numberDecimal'], $apply_default_currency['decimalSeparator'], $apply_default_currency['thousandSeparator'] );
	...
	public function admin_enqueue_scripts( $page ) {
		if ( 'toplevel_page_dokan' === $page ) {
			wp_enqueue_style(
				'yay-currency-dokan-admin-style',
				YAY_CURRENCY_PLUGIN_URL . 'src/compatibles/dokan/yay-dokan-admin.css',
				array(),
	...
	public function enqueue_scripts() {
		...
		$withdraw_limit_by_currency = YayCurrencyHelper::format_price( YayCurrencyHelper::calculate_price_by_currency( $withdraw_limit, false, $this->apply_currency ) );
		...
		global $wp;
		$order_details_area   = isset( $_REQUEST['order_id'] ) && isset( $_REQUEST['_wpnonce'] ) ? sanitize_text_field( $_REQUEST['order_id'] ) : false;
		$data_localize_script = array(
			'ajax_url'                => admin_url( 'admin-ajax.php' ),
			...
			'order_details_area'      => $order_details_area ? 'yes' : 'no',
			'order_id'                => $order_details_area ? $order_details_area : null,
			'earning_from_order_text' => '<span>' . __( 'Earning From Order:', 'dokan-lite' ) . '</span>',
		...
		if ( isset( $wp->query_vars['withdraw'] ) ) {
			...
			if ( ! empty( $last_withdraw ) ) {
				$last_payment_withdraw_currency = $this->convert_price_to_default_currency( $last_withdraw[0]->amount );
				if ( $this->default_currency !== $this->apply_currency['currency'] ) {
					$last_payment_by_currency       = YayCurrencyHelper::calculate_price_by_currency( $last_withdraw[0]->amount, false, $this->apply_currency );
					$last_payment_by_currency       = YayCurrencyHelper::format_price( $last_payment_by_currency );
	...
	public function wp_head() {
		?>
		<div class="yay-dokan-request-refund-wrapper">
			<div id="yay-dokan-request-refund-popup" class="iziModal transitionIn comingIn">
				<div class="iziModal-header iziModal-noSubtitle yay-dokan-request-refund-header">
					<h2 class="iziModal-header-title"><i class="fa fa-money" aria-hidden="true"></i>&nbsp;<?php echo esc_html__( 'YayCurrency alert', 'yay-currency' ); ?></h2>
					<div class="iziModal-header-buttons yay-dokan-request-refund-close-button">
						<a href="javascript:void(0)" class="iziModal-button iziModal-button-close"></a>
					</div>
				</div>
				<div class="iziModal-wrap yay-dokan-request-refund-content transitionIn comingIn">
					<div class="iziModal-content">
						<div class="white-popup dokan-request-refund-popup-container">
							<div id="dokan-request-refund-popup-form">
								<div class="dokan-alert dokan-alert-danger yay-dokan-request-refund-alert-message">
									<strong class="yay-dokan-request-refund-alert-text"></strong>
								</div>
			...
			<div class="yay-dokan-request-refund-overlay"></div>
		<div>
		<?php
	}

	public function custom_currency_symbol( $symbol, $apply_currency ) {
		global $wp;
		$doing_action_flag                = doing_action( 'dokan_send_withdraw_request_popup_form_content' ) || doing_action( 'dokan_after_listing_product' ) || doing_action( 'dokan_withdraw_content_after_last_payment_section' ) || doing_action( 'dokan_withdraw_request_approved' ) || doing_action( 'dokan_reverse_withdrawal_content' );
		$is_dokan_dashboard_page_requests = isset( $wp->query_vars['withdraw-requests'] ) || isset( $wp->query_vars['coupons'] );
		if ( $doing_action_flag || $is_dokan_dashboard_page_requests || isset( $_REQUEST['product_id'] ) && isset( $_REQUEST['_dokan_edit_product_nonce'] ) ) {
			$symbol = YayCurrencyHelper::get_symbol_by_currency_code( $this->default_currency );
		}
		return $symbol;
	}

	public function get_order_currency( $order_id, $order = false ) {
		$order = ! $order ? wc_get_order( $order_id ) : $order;
		if ( ! $order ) {
			return false;
		}
		$currency_code  = Helper::check_custom_orders_table_usage_enabled() ? $order->get_currency() : get_post_meta( $order_id, '_order_currency', true );
		$order_currency = YayCurrencyHelper::filtered_by_currency_code( $currency_code, $this->converted_currency );
		return $order_currency ? $order_currency : reset( $this->converted_currency );
	}
	...
	public function calculate_price_apply_currency_from_order_currency( $price, $order_currency, $only_get_price_default = false ) {
		...
		if ( $this->default_currency !== $order_currency['currency'] ) {
			...
			$price    = YayCurrencyHelper::round_line_tax( $price / $rate_fee, false );
		}

		return $only_get_price_default ? $price : YayCurrencyHelper::calculate_price_by_currency( $price, false, $this->apply_currency );
	}

	public function custom_formatted_order_subtotal( $subtotal, $compound, $order ) {
		...
		if ( 'incl' === $tax_display ) {
			...
			$order_subtotal += wc_round_tax_total( $subtotal_taxes );
		}

		$order_subtotal = $this->convert_value_by_order_id( $order_subtotal, $order->get_id() );
		if ( -1 == $order_subtotal ) {
			return $subtotal;
		}

		$subtotal = YayCurrencyHelper::format_price( $order_subtotal );
	...
	public function custom_woocommerce_order_discount_to_display( $discount, $order ) {
		$tax_display = get_option( 'woocommerce_tax_display_cart' );
		if ( 'excl' === $tax_display ) {
			$total_discount = (float) $order->get_discount_total();
		} else {
			$total_discount = (float) $order->get_discount_total() + (float) $order->get_discount_tax();
		}

		$total_discount = $this->convert_value_by_order_id( $total_discount, $order->get_id() );
		if ( -1 == $total_discount ) {
			return $discount;
		}

		return YayCurrencyHelper::format_price( $total_discount );
	}

	public function custom_woocommerce_order_shipping_to_display( $shipping, $order, $tax_display ) {
		$order_shipping = (float) $order->get_shipping_total();
		if ( 0 < abs( $order_shipping ) ) {
			$order_currency = $this->get_order_currency( $order->get_id(), $order );
			if ( ! $order_currency ) {
				return $shipping;
			}
			if ( 'excl' === $tax_display ) {
				// Show shipping excluding tax.
				$shipping = $this->calculate_price_apply_currency_from_order_currency( $order_shipping, $order_currency );
				$shipping = YayCurrencyHelper::format_price( $shipping );
				...
				$shipping = $order_shipping + (float) $order->get_shipping_tax();
				$shipping = $this->calculate_price_apply_currency_from_order_currency( $shipping, $order_currency );
				$shipping = YayCurrencyHelper::format_price( $shipping );
	...
	public function get_withdraw_by_seller_id( $seller_id, $balance_date ) {
		...
		foreach ( $results as $value ) {
			if ( 'dokan_refund' === $value->trn_type ) {
				$order_currency = $this->get_order_currency( $value->trn_id );
				if ( ! $order_currency ) {
					continue;
				}
	...
	public function get_debit_balance( $seller_id, $balance_date ) {
		...
		foreach ( $balances as $balance ) {
			$balance_debit = $this->convert_value_by_order_id( $balance->debit, $balance->trn_id );
			if ( -1 == $balance_debit ) {
				continue;
			}
			$debit += $balance_debit;
	...
	public function get_credit_balance( $seller_id, $balance_date ) {
		...
		foreach ( $balances as $balance ) {
			$balance_credit = $this->convert_value_by_order_id( $balance->credit, $balance->trn_id );
			if ( -1 == $balance_credit ) {
				continue;
			}
	...
	public function custom_dokan_seller_total_sales( $net_sales, $seller_id ) {
		...
		foreach ( $dokan_orders as $dokan_order ) {
			$net_amount = $this->convert_value_by_order_id( $dokan_order->net_amount, $dokan_order->order_id );
			if ( -1 == $net_amount ) {
				continue;
			}
	...
	public function custom_dokan_get_seller_earnings( $earning, $seller_id ) {
		...
		return YayCurrencyHelper::format_price( $earning );

	}

	public function custom_woocommerce_get_formatted_order_total( $formatted_total, $order, $tax_display, $display_refunded ) {
		...
		if ( $order_currency['currency'] === $this->apply_currency['currency'] ) {
			$formatted_total = YayCurrencyHelper::format_price( $total );
		} else {
			$total           = $this->calculate_price_apply_currency_from_order_currency( $total, $order_currency );
			$formatted_total = YayCurrencyHelper::format_price( $total );
		}

		if ( $order->has_status( 'refunded' ) ) {
			$formatted_total = YayCurrencyHelper::format_sale_price( $total, 0 );
		}

		if ( wc_tax_enabled() && 'incl' === $tax_display ) {
			$total_tax = $order->get_total_tax();
			if ( $order_currency['currency'] === $this->apply_currency['currency'] ) {
				$formatted_tax = YayCurrencyHelper::format_price( $total_tax );
			} else {
				$total_tax     = $this->calculate_price_apply_currency_from_order_currency( $total_tax, $order_currency );
				$formatted_tax = YayCurrencyHelper::format_price( $total_tax );
			}
	...
	public function custom_dokan_get_formatted_seller_balance( $earning, $seller_id ) {
		...
		$balance_currency = YayCurrencyHelper::format_price( YayCurrencyHelper::calculate_price_by_currency( $balance, false, $this->apply_currency ) );

		if ( $this->default_currency !== $this->apply_currency['currency'] ) {
			$balance_currency = $this->convert_price_to_default_currency( $balance ) . YayCurrencyHelper::converted_approximately_html( $balance_currency );
		}
	...
	public function custom_dokan_get_seller_balance( $earning, $seller_id ) {
		...
		$earning  = $this->get_earning_by_seller_id( $seller_id, $on_date );
		$withdraw = $this->get_withdraw_by_seller_id( $seller_id, $on_date );
		if ( $this->default_currency !== $this->apply_currency['currency'] ) {
			$earning = $earning / YayCurrencyHelper::get_rate_fee( $this->apply_currency );
		}
		$earning = isset( $wp->query_vars['reports'] ) ? YayCurrencyHelper::calculate_price_by_currency( $earning - $withdraw, false, $this->apply_currency ) : $earning - $withdraw;
		return $earning;
	}

	public function custom_format_order_item_totals( $total_rows, $order, $tax_display ) {
		$order_currency = $this->get_order_currency( $order->get_id(), $order );
		if ( ! $order_currency ) {
			return $total_rows;
		}
		...
		if ( 'excl' === $tax_display && wc_tax_enabled() ) {
			if ( 'itemized' === get_option( 'woocommerce_tax_total_display' ) ) {
				foreach ( $order->get_tax_totals() as $code => $tax ) {
					$tax_amount                            = $this->calculate_price_apply_currency_from_order_currency( $tax->amount, $order_currency );
					$total_rows[ sanitize_title( $code ) ] = array(
						'label' => $tax->label . ':',
						'value' => YayCurrencyHelper::format_price( $tax_amount ),
					);
				}
			} else {

				$tax_amount        = $this->calculate_price_apply_currency_from_order_currency( $order->get_total_tax(), $order_currency );
		...
		if ( $fees ) {
			foreach ( $fees as $id => $fee ) {
				...
				$fee_amount                            = 'excl' === $tax_display ? $fee->get_total() : $fee->get_total() + $fee->get_total_tax();
				$fee_amount                            = $this->calculate_price_apply_currency_from_order_currency( $fee_amount, $order_currency );
				$total_rows[ 'fee_' . $fee->get_id() ] = array(
					'label' => $fee->get_name() . ':',
					'value' => YayCurrencyHelper::format_price( $fee_amount ),
				);
		...
		if ( method_exists( $order, 'get_refunds' ) ) {
			...
			foreach ( $refunds as $id => $refund ) {
				$amount                        = $this->calculate_price_apply_currency_from_order_currency( $refund->get_amount(), $order_currency );
				$total_rows[ 'refund_' . $id ] = array(
					'label' => $refund->get_reason() ? $refund->get_reason() : __( 'Refund', 'woocommerce' ) . ':',
					'value' => YayCurrencyHelper::format_price( $amount ),
				);
			}
		}
	...
	public function custom_report_chart( $rows, $data ) {
		...
		foreach ( $rows as $key => $value ) {
			...
			$total_sales = $this->convert_value_by_order_id( (float) $value->meta_value, $value->post_id );
	...
	public function custom_report_by_sales_shipping( $seller_id, $start_date, $end_date ) {
		...
		if ( $rows ) {
			foreach ( $rows as $key => $value ) {
				...
				$order_total_sales    = $this->convert_value_by_order_id( (float) $value->sales, $value->order_id );
				$order_total_shipping = $this->convert_value_by_order_id( (float) $value->shipping, $value->order_id );
	...
	public function custom_report_total_refund( $seller_id, $start_date, $end_date ) {
		...
		foreach ( $results as $value ) {
			$order_currency = $this->get_order_currency( $value->order_id );
			if ( ! $order_currency ) {
				continue;
			}
			$total_refund += $this->calculate_price_apply_currency_from_order_currency( (float) $value->refund_amount, $order_currency );
		}
		return $total_refund;
	}
	...
	public function custom_report_by_coupons( $seller_id, $start_date, $end_date ) {
		...
		if ( $rows ) {
			foreach ( $rows as $value ) {
				...
				$total_coupons += $this->calculate_price_apply_currency_from_order_currency( (float) $value->meta_value, $order_currency );
			}
		}
		...
		if ( isset( $wp->query_vars['reports'] ) ) {
			...
			$order_items           = $wpdb->get_results(
				$wpdb->prepare(
					...
					gmdate( 'Y-m-d', strtotime( '+1 day', strtotime( $end_date ) ) )
				)
			);
			...
			foreach ( $order_items as $key => $item ) {
				...
				$order_currency = $this->get_order_currency( $item->order_id );
				if ( ! $order_currency ) {
					continue;
				}
	...
	public function get_all_data_reports( $from = false, $to = false, $seller_id = false ) {
		...
		$apply_default_currency        = YayCurrencyHelper::filtered_by_currency_code( $this->default_currency, $this->converted_currency );
		$this_month_order_total_html   = $this->get_format_price_by_default_currency( $this_month_order_total, $apply_default_currency );
		$last_month_order_total_html   = $this->get_format_price_by_default_currency( $last_month_order_total, $apply_default_currency );
		$this_month_earning_total_html = $this->get_format_price_by_default_currency( $this_month_earning_total, $apply_default_currency );
		$last_month_earning_total_html = $this->get_format_price_by_default_currency( $last_month_earning_total, $apply_default_currency );
		...
		if ( $from && $to ) {
			...
			$this_period_order_total_html   = $this->get_format_price_by_default_currency( $this_period_order_total, $apply_default_currency );
			$this_period_total_orders_html  = $this->get_format_price_by_default_currency( $this_period_total_orders, $apply_default_currency );
			$this_period_earning_total_html = $this->get_format_price_by_default_currency( $this_period_earning_total, $apply_default_currency );
	...
	public function get_rate_fee_from_currency_code_by_payment_gateway( $currency_gateway ) {
		$gateway_rate_fee       = false;
		$gateway_apply_currency = YayCurrencyHelper::filtered_by_currency_code( $currency_gateway, $this->converted_currency );
		if ( ! $gateway_apply_currency ) {
			$currency_params_template = array(
				'$src'  => $this->default_currency,
				'$dest' => $currency_gateway,
			);
			$exchange_data            = Helper::get_exchange_rates( $currency_params_template );
			if ( 200 === $exchange_data['response']['code'] ) {
				$decoded_exchange_data = json_decode( $exchange_data['body'] );
				if ( isset( $decoded_exchange_data->chart->result[0]->indicators->quote[0]->close ) ) {
					$gateway_rate_fee = $decoded_exchange_data->chart->result[0]->indicators->quote[0]->close[0];
				} else {
					$gateway_rate_fee = $decoded_exchange_data->chart->result[0]->meta->previousClose;
				}
			}
		} else {
			$gateway_rate_fee = $gateway_apply_currency['currency'] === $this->default_currency ? 1 : YayCurrencyHelper::get_rate_fee( $gateway_apply_currency );
		}
		return $gateway_rate_fee;
	}
	...
	public function custom_hide_display_stripe_order( $flag, $order_id ) {
		if ( $this->is_dokan_edit_order_page() ) {
			$flag = true;
		}
		return $flag;
	}

	public function custom_stripe_order_totals_after_total( $wc_order ) {

		if ( class_exists( 'WC_Gateway_Stripe' ) ) {
			$currency = \WC_Stripe_Helper::get_stripe_currency( $wc_order );
			if ( $currency ) {
				$rate_fee = $this->get_rate_fee_from_currency_code_by_payment_gateway( $currency );
				$fee      = \WC_Stripe_Helper::get_stripe_fee( $wc_order );
				if ( $fee ) {
					$fee = YayCurrencyHelper::calculate_price_by_currency( $fee / $rate_fee, false, $this->apply_currency );
					echo '<tr><td class="label stripe-fee">' . esc_html__( 'Stripe Fee:', 'woocommerce-gateway-stripe' ) . '</td><td width="1%"></td>';
					echo '<td class="total">-' . wp_kses( wc_price( $fee ), $this->get_allow_html() ) . '</td></tr>';
				}
				$net = \WC_Stripe_Helper::get_stripe_net( $wc_order );
				if ( $net ) {
					$net = YayCurrencyHelper::calculate_price_by_currency( $net / $rate_fee, false, $this->apply_currency );
					echo '<tr><td class="label stripe-payout">' . esc_html__( 'Stripe Payout:', 'woocommerce-gateway-stripe' ) . '</td><td width="1%"></td>';
					echo '<td class="total">-' . wp_kses( wc_price( $net ), $this->get_allow_html() ) . '</td></tr>';
				}
			}
		}
	...
	public function custom_paypal_order_totals_after_total( $wc_order ) {
		if ( class_exists( 'WooCommerce\PayPalCommerce\WcGateway\Gateway\PayPalGateway' ) ) {
			...
			if ( is_array( $breakdown ) ) {
				$fee      = isset( $breakdown['paypal_fee'] ) && $breakdown['paypal_fee'] ? $breakdown['paypal_fee'] : false;
				$net      = isset( $breakdown['net_amount'] ) && $breakdown['net_amount'] ? $breakdown['net_amount'] : false;
				$rate_fee = false;
				if ( $fee || $net ) {
					$currency = isset( $fee['currency_code'] ) ? $fee['currency_code'] : ( isset( $net['currency_code'] ) ? $net['currency_code'] : false );
					$rate_fee = $this->get_rate_fee_from_currency_code_by_payment_gateway( $currency );
					if ( $fee ) {
						$fee_value = YayCurrencyHelper::calculate_price_by_currency( $fee['value'] / $rate_fee, false, $this->apply_currency );
						echo '<tr><td class="label">' . esc_html__( 'PayPal Fee:', 'woocommerce-paypal-payments' ) . '</td><td width="1%"></td>
							<td class="total"> - ' . wp_kses( wc_price( $fee_value ), $this->get_allow_html() ) . '</td></tr>';

					}
					if ( $net ) {
						$net_value = YayCurrencyHelper::calculate_price_by_currency( $net['value'] / $rate_fee, false, $this->apply_currency );
						echo '<tr><td class="label">' . esc_html__( 'PayPal Payout:', 'woocommerce-paypal-payments' ) . '</td><td width="1%"></td>
							<td class="total"> - ' . wp_kses( wc_price( $net_value ), $this->get_allow_html() ) . '</td></tr>';
					}
				}
			}
		}
	...
	// PAYPAL
	public function custom_paypal_payments_show_fees( $flag, $order ) {
	...
	public function custom_admin_order_totals_after_total( $order_id ) {
		if ( $this->is_dokan_edit_order_page() ) {
			...
			// PAYPAL
			$this->custom_paypal_order_totals_after_total( $wc_order );
			// STRIPE
			$this->custom_stripe_order_totals_after_total( $wc_order );
		}

	}

	public function custom_yay_dokan_custom_order_details() {
		$nonce = isset( $_POST['_nonce'] ) ? sanitize_text_field( $_POST['_nonce'] ) : false;
		if ( ! $nonce || ! wp_verify_nonce( sanitize_key( $nonce ), 'yay-currency-dokan-nonce' ) ) {
			wp_send_json_error( array( 'message' => __( 'Nonce invalid', 'yay-currency' ) ) );
		}
		...
		$order_currency = $this->get_order_currency( $order_id, $order );
		if ( ! $order_currency ) {
			wp_send_json_error( array( 'message' => __( 'Order doesn\'t exist', 'yay-currency' ) ) );
		}
		...
		if ( wc_tax_enabled() ) {
			$order_taxes  = $order->get_taxes();
			$check_item   = current( $order_items );
			$tax_data     = maybe_unserialize( isset( $check_item['line_tax_data'] ) ? $check_item['line_tax_data'] : '' );
			$legacy_order = ! empty( $order_taxes ) && empty( $tax_data ) && ! is_array( $tax_data );
		} else {
			$legacy_order = false;
				...
				case 'line_item':
					...
					if ( isset( $item['line_total'] ) ) {
						if ( isset( $item['line_subtotal'] ) && $item['line_subtotal'] !== $item['line_total'] ) {
							$line_subtotal   = $this->calculate_price_apply_currency_from_order_currency( $item['line_subtotal'], $order_currency );
							$line_cost_html .= wp_kses_post( '<del>' . YayCurrencyHelper::format_price( $line_subtotal ) . '</del> ' );
							$item_cost_html .= wp_kses_post( '<del>' . YayCurrencyHelper::format_price( $line_subtotal / $quantity ) . '</del> ' );
						}
						$line_total      = $this->calculate_price_apply_currency_from_order_currency( $item['line_total'], $order_currency );
						$line_cost_html .= wp_kses_post( YayCurrencyHelper::format_price( $line_total ) );
						$item_cost_html .= wp_kses_post( YayCurrencyHelper::format_price( $line_total / $quantity ) );
						$refunded        = $order->get_total_refunded_for_item( $item_id );
						if ( $refunded ) {
							$refunded_html   = $this->calculate_price_apply_currency_from_order_currency( $refunded, $order_currency );
							$line_cost_html .= ' <small class="refunded">-' . YayCurrencyHelper::format_price( $refunded_html ) . '</small>';
						}
					}
					$tax_item_total_html = YayCurrencyHelper::format_price( $this->calculate_price_apply_currency_from_order_currency( $item['total_tax'], $order_currency ) );
					if ( empty( $legacy_order ) && wc_tax_enabled() ) {
						$line_tax_data = isset( $item['line_tax_data'] ) ? $item['line_tax_data'] : '';
						$tax_data      = maybe_unserialize( $line_tax_data );
						foreach ( $order_taxes as $tax_item ) {
							...
							$tax_item_id         = $tax_item['rate_id'];
							$tax_item_total      = isset( $tax_data['total'][ $tax_item_id ] ) ? $tax_data['total'][ $tax_item_id ] : '';
							$tax_item_total_html = ! empty( $tax_item_total ) ? YayCurrencyHelper::format_price( $this->calculate_price_apply_currency_from_order_currency( wc_round_tax_total( $tax_item_total ), $order_currency ) ) : '&ndash;';
							$refunded            = $order->get_tax_refunded_for_item( $item_id, $tax_item_id );
							if ( $refunded ) {
								$refunded_html       = YayCurrencyHelper::format_price( $this->calculate_price_apply_currency_from_order_currency( $refunded, $order_currency ) );
								$tax_item_total_html = $tax_item_total_html . ' <small class="refunded">-' . $refunded_html . '</small>';
							}
						}
					}

					$data[ $item_id ] = array(
						'item_cost_html' => $item_cost_html,
						'line_cost_html' => $line_cost_html,
						'line_tax_html'  => $tax_item_total_html,
					);
					break;
				case 'fee':
					$fee_line_total   = isset( $item['line_total'] ) ? $item['line_total'] : false;
					$fee_line_total   = $this->calculate_price_apply_currency_from_order_currency( $fee_line_total, $order_currency );
					$fees[ $item_id ] = array(
						'line_cost_total' => YayCurrencyHelper::round_line_tax( $fee_line_total, false ),
					);
					if ( 'yes' === get_option( 'woocommerce_calc_taxes' ) ) {
						$fee_line_tax_total                 = isset( $item['line_tax'] ) ? $item['line_tax'] : 0;
						$fee_line_tax_total                 = $this->calculate_price_apply_currency_from_order_currency( $fee_line_tax_total, $order_currency );
						$fees[ $item_id ]['line_tax_total'] = $fee_line_tax_total;
					}
					break;
			}
		}
		...
		$json_data = array(
			...
			'fee_details' => $fees ? wp_json_encode( $fees ) : false,
		);

		if ( $order_currency['currency'] === $this->apply_currency['currency'] ) {
			$json_data['allow_refund'] = true;
		} else {
			$json_data['yay_currency_alert_refund'] = __( 'Refund cannot be requested, please switch currency about', 'yay-currency' ) . ' [' . $order_currency['currency'] . '] ' . __( 'to sync with this order items currency.', 'yay-currency' );
		}

		if ( class_exists( 'Dokan_Pro' ) ) {
			$order_totals              = $this->custom_table_order_totals( $order, $order_currency );
			$json_data['order_totals'] = $order_totals;

			$shipping_totals = $this->custom_order_shipping_totals( $order, $order_currency );
			if ( $shipping_totals ) {
				$json_data['shipping_totals'] = $shipping_totals;
			}

			$fee_totals = $this->custom_order_fee_totals( $order, $order_currency );
			if ( $shipping_totals ) {
				$json_data['fee_totals'] = $fee_totals;
			}

			$refund_totals = $this->custom_order_refund_totals( $order, $order_currency );
			if ( $refund_totals ) {
				$json_data['refund_totals']        = $refund_totals['totals'];
				$json_data['order_total_refunded'] = $refund_totals['total_refunded'];
			}
		}
	...
	public function custom_yay_dokan_approximately_price() {
		...
		wp_send_json_success(
			array(
				'price_html' => YayCurrencyHelper::converted_approximately_html( $this->custom_formatted_price_by_currency( $price ) ),
			)
		);
	}
		...
		if ( $start_date && $end_date && $seller_id ) {
			...
			foreach ( $statements as $key => $statement ) {
				...
				if ( in_array( $statement_type, array( 'dokan_orders', 'dokan_refund' ) ) ) {
					$statement_debit  = $this->convert_value_by_order_id( $statement->debit, $statement->trn_id );
					$statement_credit = $this->convert_value_by_order_id( $statement->credit, $statement->trn_id );
					...
					$debit  = YayCurrencyHelper::calculate_price_by_currency( $statement->debit, false, $this->apply_currency );
					$credit = YayCurrencyHelper::calculate_price_by_currency( $statement->credit, false, $this->apply_currency );
				}
				$total_debit                += $debit;
				$total_credit               += $credit;
				$balance                    += abs( $debit - $credit );
				$results[ $index ]['debit']  = YayCurrencyHelper::format_price( $debit );
				$results[ $index ]['credit'] = YayCurrencyHelper::format_price( $credit );
			...
			wp_send_json_success(
				array(
					'statements'    => $results,
					'total_debit'   => YayCurrencyHelper::format_price( $total_debit ),
					'total_credit'  => YayCurrencyHelper::format_price( $total_credit ),
					'total_balance' => YayCurrencyHelper::format_price( $balance ),
				)
			);
		}
	...
	public function get_all_refund_by_status( $order_ids = array(), $status = 0 ) {
		...
		$refunds = array_filter( $refunds, fn ( $value) =>  in_array( $value->order_id, $order_ids ) );
		return $refunds;
	}
